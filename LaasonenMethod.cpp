	...
	for (int j = 1; j < (0.5/deltaT) + 1; j++) {// the limit for this loop is the number of timesteps needed to get to t = 0.5.
		...
		for (int i = 0; i < numberPosPerProcess - lastSpaces - (2*isLast); i++) {
			...
			if (i == 0 && myrank == 0) {// first value of the d vector calculation and simultaneously the forward substitution phase of the algorithm.
				...
				d[myrank*numberPosPerProcess + i] = previousX[myrank*numberPosPerProcess + i + 1] - aCoef*x[0];
				...
				d[myrank*numberPosPerProcess + i] = previousX[myrank*numberPosPerProcess + i + 1] - cCoef*x[numberPosPerProcess - lastSpaces - 1];
				...
				d[myrank*numberPosPerProcess + i] = previousX[myrank*numberPosPerProcess + i + 1];
		...
		if ((n-1) != 1)
			MPI_Allgatherv(&x[0], counts[myrank], MPI_DOUBLE, &previousX[0], counts, disps, MPI_DOUBLE, MPI_COMM_WORLD);
		else
			previousX[1] = x[1];
